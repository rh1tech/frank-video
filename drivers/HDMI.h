...
/* Schedule a buffer swap to happen at the next vblank instead of mid-scan.
 * The pointer is staged and the DMA IRQ swaps the live framebuffer only
 * on the vsync_handler boundary, so the entire next frame comes from the
 * new buffer with no tearing. Returns immediately; pair with
 * graphics_wait_vsync() to block until the swap has actually been applied. */
void graphics_set_buffer_at_vsync(uint8_t *buffer);

/* Block until the next vblank tick from the HDMI DMA IRQ. Use this to
 * pace the renderer to the display so we never queue swaps faster than
 * the panel can apply them. */
