...
/* Queue capacity. murmnes used 512 (sized for ~43ms of NES audio bursts).
 * frank-video pushes 1152-frame MPEG-1 audio bursts at 32-48 kHz, so we
 * size for ~3 bursts of slack to absorb the player's wall-clock-paced
 * producer cadence. Each entry is sizeof(hstx_data_island_t) = 144 bytes
 * -> 1024 entries = 144 KB SRAM. */
