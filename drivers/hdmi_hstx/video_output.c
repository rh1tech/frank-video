...
void video_output_core1_run(void)
{
    ...
    /* Give the HSTX DMA channels priority on the bus so they always have
     * pixel data ready for the FIFO -- without this the picture loses
     * sync the moment Core 0 starts heavy PSRAM/flash work. We do NOT
     * grant Core 1 priority here: this bus is used by Core 0 as well
     * (the MPEG-1 decoder lives on Core 0 in frank-video), and giving
     * Core 1 priority on top of DMA starves the decoder enough to drop
     * sustained throughput by ~7x. The scanline IRQ on Core 1 still runs
     * at IRQ priority, which is enough to keep the line buffer filled. */
