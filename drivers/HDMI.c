...
/* Staged buffer pointer for tear-free swaps. graphics_set_buffer_at_vsync()
 * writes here; the DMA IRQ's vsync_handler() promotes it into
 * graphics_buffer at the start of vblank so a swap never lands mid-frame. */
static uint8_t * volatile graphics_buffer_pending = NULL;

/* Bumped once per frame in vsync_handler(). Producers can poll this to
 * block until the next vblank without spinning on graphics_buffer_pending
 * directly. */
...
void vsync_handler() {
    /* Apply any pending tear-free buffer swap. The staged pointer is
     * written by graphics_set_buffer_at_vsync() from any core; we promote
     * it into the live framebuffer at the boundary of vblank so the next
     * frame is sourced wholly from the new buffer. */
